...
def validate_tester_ini(config: str | Path, source: str | Path | None = None) -> dict:
    ...
    if "Deposit" in tester:
        try:
            if float(tester["Deposit"]) <= 0:
                issues.append({"section": "Tester", "key": "Deposit", "message": "must be > 0"})
        except ValueError:
            issues.append({"section": "Tester", "key": "Deposit",
                           "message": f"non-numeric value '{tester['Deposit']}'"})

    if "Leverage" in tester and not _valid_leverage(tester["Leverage"]):
        issues.append({"section": "Tester", "key": "Leverage",
                       "message": f"invalid leverage '{tester['Leverage']}' (use 100 or 1:100)"})
...
def _valid_leverage(value: str) -> bool:
    text = (value or "").strip()
