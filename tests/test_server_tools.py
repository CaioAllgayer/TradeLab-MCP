...
def test_compile_invokes_metaeditor(fake_layout, tmp_path):
    ...
    def fake_run(cmd, capture_output, text, timeout):
        ...
        src.with_suffix(".ex5").write_bytes(b"BIN")
    ...
    assert out["binary_sha256"]


def test_run_backtest_missing_strategy(fake_layout):
    out = server.run_backtest(
        strategy="/no/such.mq5",
        symbol="PETR4",
        timeframe="D1",
